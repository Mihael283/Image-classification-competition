...
# ... (keep all the import statements and other classes as they are)
...
class EfficientNetV2(nn.Module):
    ...
    version = 's',
    dropout_rate = 0.2,
    num_classes = 20  # Change this to match your number of classes
    ):
    ...
    def _feature_extractor(self, version, last_channel):
        ...
        layers = []
        # Change input channels to 1 for grayscale, and adjust stride to 1 to maintain spatial dimensions
        layers.append(ConvBnAct(1, config[0][3], k_size = 3, stride = 1, padding = 1))
        
        # Modify the config to reduce the number of downsampling operations
        modified_config = [
            [1, 3, 1, 24, 24, 2, True],
            [4, 3, 2, 24, 48, 4, True],
            [4, 3, 2, 48, 64, 4, True],
        ...
        for (expansion_factor, k, stride, n_in, n_out, num_layers, use_fused) in modified_config:
        ...
        layers.append(ConvBnAct(modified_config[-1][4], last_channel, k_size = 1))   
            
        return nn.Sequential(*layers)

def MyCNN(num_classes=20):
    return EfficientNetV2(version='s', num_classes=num_classes)